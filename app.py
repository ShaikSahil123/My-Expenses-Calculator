...
tab1, tab2, tab3 = st.tabs(
    ["📊 Detailed Dashboard", "➕ Add Transaction", "🗑️ History & Delete"]
)
    ...
    with st.form("entry_form", clear_on_submit=True):
        ...
        with col_a:
            ...
            trans_type = st.selectbox(
                "What Type?", ["Expense", "Income", "Debt Given", "Debt Repaid"]
            )
        ...
        if submitted:
            # Check exactly which field is missing and tell the user
            if amount_val <= 0:
                st.error("⚠️ Error: Amount must be greater than 0.")
            elif not category_val:
                st.error(
                    f"⚠️ Error: The '{label}' field is empty. Please type something."
                )
            else:
                # If both are good, save!
...
with tab3:
    ...
    if not df.empty:
        ...
        selected_option = st.selectbox(
            "Select which entry to delete:", options=delete_options
        )
